...
//	service 클래스에서 호출되는 mapper와 테이블에 저장될 메인글 정보가 저장된 객체를 넘겨받고 메인글을 테이블에 저장하는 mvcboard.xml의 insert sql명령을 실행하는 메소드
	public void insert(SqlSession mapper, MvcboardVO vo) {
		System.out.println("MvcboardDAO 클래스의 insert()메소드");
		mapper.insert("insert", vo);
	}
//	service 클래스에서 호출되는 mapper를 넘겨받고 테이블에 저장된 전체 글의 갯수를 얻어오는 mvcboard.xml의 select sql명령을 실행하는 메소드
	public int selectCount(SqlSession mapper) {
		System.out.println("MvcboardDAO 클래스의 selectCount()메소드");
		return (int) mapper.selectOne("selectCount");
	}
//	service 클래스에서 호출되는 mapper와 1페이지의 시작-끝 인덱스가 저장된 HashMap 객체를 넘겨받고 테이블에 저장된 글 중에서 1페이지 분량의 글 목록을 얻어오는 mvcboard.xml의 파일의 select sql 명령을 실행하는 메소드
...
//	service 클래스에서 호출되는 mapper와 조회수를 증가시킬 글번호를 넘겨받고 조회수를 증가시키는 mvcboard.xml의 update sql명령을 실행하는 메소드
...
//	service 클래스에서 호출되는 mapper와 조회수를 증가시킨 글의 글번호를 넘겨받고 글 1건을 얻어오는 mvcboard.xml의 select sql명령을 실행하는 메소드 
...
//	service 클래스에서 호출되는 mapper와 삭제할 글의 글번호를 넘겨받고 글 1건을 삭제하는 mvcboard.xml의 delete sql명령을 실행하는 메소드 
	public void delete(SqlSession mapper, int idx) {
		System.out.println("MvcboardDAO 클래스의 delete()메소드");
		mapper.delete("delete", idx);
	}
//	service 클래스에서 호출되는 mapper와 수정할 정보가 저장된 객체를 넘겨받고 글 1건을 수정하는 mvcboard.xml의 update sql명령을 실행하는 메소드 
...
//	service클래스에서 호출되는 매퍼와 답글 정보가 저장된 객체를 넘겨받고 mvcboard.xml 파일의 insert sql명령을 실행하는 메소드
