	...
	<table width="1000" align="center" border="1" cellpadding="5" cellspacing="0">
		...
		<c:if test="${notices.size() != 0  && freeboardList.currentPage == 1}">
			...
			<td>
				...
				<a href="increment.jsp?idx=${notice.idx}&currentPage=${freeboardList.currentPage}" style="font-weight: bold; background-color:orange;">
				${subject}
				</a>[${notice.commentCount}]  
			...
			<td align="center">
					<c:if test="${date.year == notice.writeDate.year && date.month == notice.writeDate.month &&
					 date.date == notice.writeDate.date}">
						<fmt:formatDate value="${notice.writeDate}" pattern="HH:mm:ss"/>
					</c:if>
					<c:if test="${date.year != notice.writeDate.year || date.month != notice.writeDate.month || 
					date.date != notice.writeDate.date}">
						<fmt:formatDate value="${notice.writeDate}" pattern="yyyy-MM-dd(E) HH:mm:ss"/>
					</c:if>
