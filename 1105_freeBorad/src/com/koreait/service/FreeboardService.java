...
//	insertOK.jsp에서 호출되는 테이블에 저장할 메인글이 저장된 객체를 넘겨받고 mapper를 얻어온 후 메인글을 저장하는 DAO클래스의 insert sql 명령 실행메소드를 호출하는 메소드	
...
//	list.jsp에서 호출되는 브라우저에 출력할 페이지 번호를 넘겨받고 mapper를 얻어온 후 1페이지 분량의 메인글 목록을 얻어오는 DAO 클래스의 select sql 명령실행 메소드를 호출하는 메소드 
...
//	incerement.jsp에서 호출, 조회수를 증가시킬 글번호를 넘겨받고 mapper를 얻어온 후 메인글의 조회수를 증가키시는 DAO클래스의 update sql명령을 실행하는 메소드를 호출하는 메소드
...
//	selectByIdx.jsp에서 호출되는 화면에 표시할 메인의 글번호와 mapper를 얻어온 후 메인글 1건을 얻어오는 DAO 클래스의 select sql명령을 실행하는 메소드를 호출하는 메소드
...
//	deleteOK.jsp에서 호출되는 삭제할 글번호를 넘겨받고 mapper를 얻어온 후 메인글 1건을 삭제하는 DAO 클래스의 delete sql 명령을 실행하는 메소드를 호출하는 메소드
...
//	updateOK.jsp에서 호출되는 수정할 글번호를 넘겨받고 mapper를 얻어온 후 메인글 1건을 수정하는 DAO 클래스의 update sql 명령을 실행하는 메소드를 호출하는 메소드
...
//	list.jsp에서 호출되는 mapper를 얻어온후 공지글 전체를 얻어오는 DAO 클래스의 select sql명령을 실행하는 메소드를 호출하는 메소드
	public ArrayList<FreeboardVO> selectNotice() {
		System.out.println("FreeboardService 클래스의 selectNotice() 메소드");
		SqlSession mapper = MySession.getSession();
		
		ArrayList<FreeboardVO> notices =  FreeboardDAO.getInstance().selectNotice(mapper);
