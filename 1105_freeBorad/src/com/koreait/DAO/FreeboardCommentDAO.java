...
//	service 클래스가 호출하는 mapper와 댓글데이터가 저장된 객체를 넘겨받고 freeboardcomment.xml의 insert sql 명령을 실행하는 메소드
...
//	service 클래스에서 mapper와 메인글의 글번호를 넘겨받고 메인글의 댓글 개수를 얻어오는 freeboardcomment.xml의 select sql 명령을 실행하는 메소드
...
//	service 클래스에서 mapper와 메인글의 글번호를 넘겨받고 메인글의 댓글 목록을 얻어오는 freeboardcomment.xml의 select sql 명령을 실행하는 메소드
...
//	service 클래스에서 mapper와 삭제 또는 수정할 댓글의 글번호를 넘겨받고 댓글 1건을 얻어오는 freeboardcomment.xml의 select sql 명령을 실행하는 메소드
...
//	service 클래스에서 mapper와 삭제할 댓글의 글번호를 넘겨받고 댓글 1건을 삭제하는 freeboardcomment.xml의 delete sql 명령을 실행하는 메소드
	public void deleteComment(SqlSession mapper, int idx) {
		
		mapper.delete("deleteComment", idx);
		
	}
//	service 클래스에서 mapper와 수정할 댓글의 정보가 저장된 객체를 넘겨받고 댓글 1건을 수정하는 freeboardcomment.xml의 update sql 명령을 실행하는 메소드
