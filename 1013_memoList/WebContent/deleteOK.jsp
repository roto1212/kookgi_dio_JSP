...
<%
	...
	//	memoDelete.jsp에서 넘어오는 삭제할 글번호(idx), 삭제 후 돌아갈 페이지 번호(currentPage), 삭제하기 위해 입력한 비밀번호(pw)를 받는다.
...
//		비밀번호가 일치하므로 글을 삭제하고 memoList4.jsp로 돌아간다.
		sql = "delete from memolist where idx=?";
		pstmt = conn.prepareStatement(sql);
		pstmt.setInt(1, idx);
		pstmt.executeUpdate();
		out.println("alert('" + idx + "번째 글 삭제 완료')");
	} else {
