...
//		삭제할 글번호와 삭제후 돌아갈 페이지번호가 모두 숫자인가 검사한다.
		try {
...
<form action="deleteOK.jsp" method="post">
	...
	<tr>
		<td colspan="2" align="center">
			...
			<input type="button" value="돌아가기3 새로고침 된 지정페이지로" onclick="location.href='memoList4.jsp?currentPage=<%=currentPage%>'"/> <!-- 삭제페이지 들어간 사이에 변경된 사항을 적용 -->
			
		</td>
	</tr>
	</table>
	<!-- 삭제할 글번호와 삭제 작업후 돌아갈 페이지 번호도 deleteOK.jsp로 넘겨줘야한다. -->
...
<% 
	...
	//		삭제할 글이 테이블에 존재하지 않기 때문에 에러메시지를 출력하고 memoList4.jsp로 돌려보낸다.
				out.println("<script>");		
...
//		삭제할 글번호와 삭제 후 돌아갈 페이지 번호가 모두 숫자로 넘어오지 않으면 에러메시지를 출력하고 memoList4.jsp로 돌려보낸다.
			out.println("<script>");		
			out.println("alert('삭제할 글번호와 돌아갈 페이지가 올바르지 않습니다.')");
			out.println("location.href='memoList4.jsp'");
			out.println("</script>");		
			
		}
	} else {
//		삭제할 글번호와 삭제 후 돌아갈 페이지 번호가 모두 넘어오지 않으면 에러메시지를 출력하고 memoList4.jsp로 돌려보낸다.
		out.println("<script>");		
		out.println("alert('삭제할 글번호와 돌아갈 페이지가 올바르지 않습니다.')");
		out.println("location.href='memoList4.jsp'");
		out.println("</script>");		
