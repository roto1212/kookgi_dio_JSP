...
//	enctype이 multipart/form-data로 설정된 폼에서 넘어오는 데이터를 받으려면 일반적인 request로 받을 수 없고 multipart를 지원하는 객체로 받아야한다.
	MultipartRequest mr = new MultipartRequest(
			request, 							//요청객체
			application.getRealPath("/upload"),	//업로드 되는 파일이 실제로 저장될 경로
			5 * 1024 * 1024,					//업로드 되는 파일의 최대크기 => 바이트 단위로 지정한다.
			"UTF-8",							//문자 인코딩방식
...
//	실제로 디스크에 저장된 파일이름을 얻어온다.
	File file = mr.getFile("filename");
	out.println("업로드된 실제 파일 이름: " + file.getName() + "<br/>");
%>
