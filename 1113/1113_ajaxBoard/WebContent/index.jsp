...
<script type="text/javascript">
	...
	// XMLHttpRequest : js Object로 http를 통한 송수신을 지원해주는 객체
...
//	alert('searchFunction() 함수 실행');
		...
		// XMLHttpRequest 객체.open('요청방식', url, 동기방법);
		// 동기방법: 동기식 => false || 비동기식 => true
		// encodeURIComponent(): 문자열을 유니코드로 인코딩한다. 
		// decodeURIComponent(): 인코딩한 문자열을 디코딩한다. 
	...
	//alert('searchFunction() 함수에서 ajax가 완료되면 자동으로 실행되는 함수');

	//onreadystatechange로 지정한 콜백함수의 readyState
	// 0 : 실행되지 않음
	// 1 : 로드 중
	// 2 : 로딩 됨
	// 3 : 통신 됨
	...
	//onreadystatechange로 지정한 콜백함수의 status
	// 200 : 수신성공
	// 300 : 금지
	// 400 : 페이지 없음
	// 500 : 서버오류
	...
	//	console.log("status: " + searchRequest.status);
		// 통신이 완료됨을 확인하고 필요한 작업을 실행한다. (4, 200)
...
//		서블릿에서 수신한 데이터를 출력하기 위해 기존에 표시되던 데이터를 제거한다.
			...
			// eval(): 인수로 지정된 문자열로 구성된 js 를 실행한다.
			var object = eval("(" + searchRequest.responseText + ")"); // 주의 요망
			// JSON객체에서 result라는 이름이 지정된 데이터를 얻어온다. => 화면에 출력할 데이터
...
// 페이지가 로드되지마자 화면에 전체 데이터가 보여지게 하기 위해서 onload 이벤트에서 searchFunction() 함수를 실행한다.
onload = function() {
	searchFunction();
}
	...
	<table class="table" align="center" style="text-align: center; border: 1px solid black;">
		...
		<tbody>
			...
			<tr>
				...
				<td><input id="e-mail" size="20" type="text" class="form-control"/></td>
			</tr>

			<tr>
				<td colspan="2" > <button type="button" class="btn btn-primary pull-right">입력</button></td>
			</tr>
		</tbody>
	</table>
</div>
