...
<style type="text/css">
	...
	/*  .클래스: 태그에 상관없이 클래스가 걸려있는 모든 태크에 스타일적용*/
	.select {	
...
//	어린이날이 토요일 또는 일요일과 겹치는 경우 그 다음 첫번째 평일을 대체공휴일로 한다.
		boolean flag = false;
		int child = 0;
		if (MyCalendar.weekDay(year, 5, 5) == 6){ //어린일 날이 토요일
			flag = true;
			child = 7;
		} else if (MyCalendar.weekDay(year, 5, 5) == 0){ //어린이날이 일요일
			flag = true;
			child = 6;
		}
...
//	양력 공휴일을 표시한다.
		if (month == 1 && i == 1){
			...
			out.println("<td class='holiday'>"+ i + "<br><span>어린이 날</span></td>");
		} else if (month == 6 && i == 6){
